...
export function Topbar({ problemTitle, startedAt, onExit }: Props) {
  ...
  const sec = now == null ? 0 : Math.max(0, Math.floor((now - startedAt) / 1000));
  const mm = now == null ? "--" : String(Math.floor(sec / 60)).padStart(2, "0");
  const ss = now == null ? "--" : String(sec % 60).padStart(2, "0");
